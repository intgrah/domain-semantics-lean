...
theorem IsDefEq.forallE_inv {Γ : List Term} {A A' B B' T : Term}
    (hΓ : Raw.WF Γ) (d : Γ ⊢ .forallE A B ≡ .forallE A' B' : T) :
    Γ ⊢ A ≡ A' type ∧ A :: Γ ⊢ B ≡ B' type ∧ A' :: Γ ⊢ B ≡ B' type :=
  have p := d.rawSoundness hΓ
  have ⟨_, _, hA, hB, _, _⟩ := p.left.ready
  have ⟨_, _, hA', hB', _, _⟩ := p.right.ready
  rawInterpret_forallE_inversion piLimit hA hB hA' hB' (fun _ ↦ ⊥)
    (p.equal (𝟙 _) (fun _ ↦ ⊥) (hΓ.bottom_admissible (𝟙 _)))

theorem IsDefEq.sort_inv {Γ : List Term} {u v : Bool} {T : Term}
    (hΓ : Raw.WF Γ) (d : Γ ⊢ .sort u ≡ .sort v : T) : u = v := by
  have h : (RawFamily.sort u).app _ (𝟙 _).op _ = (RawFamily.sort v).app _ (𝟙 _).op _ :=
    (d.rawSoundness hΓ).equal (𝟙 _) (fun _ ↦ ⊥) (hΓ.bottom_admissible (𝟙 _))
...
theorem IsDefEq.canon {Γ : List Term} {e₁ e₂ V : Term} :
    Γ ⊢ e₁ ≡ e₂ : V → ⊢ Γ → Canon Γ e₁ V ∧ Canon Γ e₂ V
...
theorem IsDefEq.type_unique {Γ : List Term} {A B : Term} (hΓ : ⊢ Γ)
    (h₁ : Γ ⊢ M : A) (h₂ : Γ ⊢ M : B) : Γ ⊢ A ≡ B type :=
  Canon.unique hΓ (h₁.canon hΓ).1 (h₂.canon hΓ).1

theorem IsDefEq.sort_unique {Γ : List Term} {u v : Bool} (hΓ : ⊢ Γ)
    (h₁ : Γ ⊢ M : .sort u) (h₂ : Γ ⊢ M : .sort v) : u = v :=
  have ⟨_, h⟩ := IsDefEq.type_unique hΓ h₁ h₂
  h.sort_inv hΓ
