...
theorem le_def {L M : ΩLower R X} :
    L ≤ M ↔ ∀ {Y : C} (f : Y ⟶ X) (a : R.obj (op Y)),
      L.mem f a → M.mem f a := Iff.rfl
...
instance (R : Cᵒᵖ ⥤ CondSemilatSup.{max u v}) (X : Cᵒᵖ) : CompleteLattice ((presheaf R).obj X) :=
...
theorem IsDirected.exists_upper {L : ΩLower R X} (hL : L.IsDirected)
    (f : Y ⟶ X) (s : Finset (R.obj (op Y))) (h : ∀ a ∈ s, L.mem f a) :
    ∃ b, L.mem f b ∧ ∀ a ∈ s, a ≤ b := by
  ...
  have ⟨b, hb⟩ := hd.finite_le (fun a : s => ⟨a, h a a.property⟩)
  exact ⟨b, b.property, fun a ha => hb ⟨a, ha⟩⟩

theorem IsDirected.exists_principal_between {L : ΩLower R X} (hL : L.IsDirected)
    {s : Finset (R.obj (op X))} (h : s.sup (principal R) ≤ L) :
    ∃ a, L.mem (𝟙 X) a ∧ s.sup (principal R) ≤ principal R a ∧ principal R a ≤ L := by
  have ⟨a, ha, hsa⟩ := hL.exists_upper (𝟙 X) s (by simpa using @h)
  exact ⟨a, ha, Finset.sup_le (α := ΩLower R X) fun b hb => principal_mono (hsa b hb),
    principal_le_iff.mpr ha⟩
...
theorem mem_sup (L M : ΩLower R X) (f : Y ⟶ X) (a : R.obj (op Y)) :
    (L ⊔ M).mem f a ↔ a ≤ ⊥ ∨ L.mem f a ∨ M.mem f a := by
  rw [sup_eq_iSup, mem_iSup]
  simp
  exact or_congr_right or_comm

theorem pullback_sup (L M : ΩLower R X) (f : Y ⟶ X) :
    (L ⊔ M).pullback f = L.pullback f ⊔ M.pullback f := by
  ext
  rw [presheaf_map_mem, mem_sup, mem_sup, presheaf_map_mem, presheaf_map_mem]

def guard (S : Sieve X) (L : ΩLower R X) : ΩLower R X where
  mem f a := a ≤ ⊥ ∨ (S f ∧ L.mem f a)
  natural f g a
    | .inl ha => .inl ((R.map g.op).map_le_bot ha)
    | .inr ⟨hS, ha⟩ => .inr ⟨S.downward_closed hS g, L.natural f g a ha⟩
  bottom _ := .inl le_rfl
  lower f hab
    | .inl hb => .inl (hab.trans hb)
    | .inr ⟨hS, hb⟩ => .inr ⟨hS, L.lower f hab hb⟩

@[simp] theorem mem_guard (S : Sieve X) (L : ΩLower R X) (f : Y ⟶ X) (a : R.obj (op Y)) :
    (guard S L).mem f a ↔ a ≤ ⊥ ∨ (S f ∧ L.mem f a) := Iff.rfl

theorem guard_mono {S T : Sieve X} {L M : ΩLower R X} (hS : S ≤ T) (hL : L ≤ M) :
    guard S L ≤ guard T M := by
  intro Y f a ha
  rw [mem_guard] at ha ⊢
  rcases ha with ha | ⟨hf, ha⟩
  · exact Or.inl ha
  · exact Or.inr ⟨hS f hf, hL f a ha⟩

theorem pullback_guard (S : Sieve X) (L : ΩLower R X) (f : Y ⟶ X) :
    (guard S L).pullback f = guard (S.pullback f) (L.pullback f) := by
  ext
  rw [presheaf_map_mem, mem_guard, mem_guard, presheaf_map_mem]
  rfl

def memSieve (L : ΩLower R X) (a : R.obj (op X)) : Sieve X where
  arrows Y f := L.mem f (R.map f.op a)
  downward_closed {Y Z f} hf g := by
    simpa [← Functor.map_comp_apply, ← op_comp] using L.natural f g _ hf

@[simp] theorem memSieve_apply (L : ΩLower R X) (a : R.obj (op X)) (f : Y ⟶ X) :
    L.memSieve a f ↔ L.mem f (R.map f.op a) := Iff.rfl

theorem memSieve_mono {L M : ΩLower R X} (h : L ≤ M) (a : R.obj (op X)) :
    L.memSieve a ≤ M.memSieve a := fun _ f hf => h f _ hf

theorem memSieve_pullback (L : ΩLower R X) (a : R.obj (op X)) (f : Y ⟶ X) :
    (L.memSieve a).pullback f = (L.pullback f).memSieve (R.map f.op a) := by
  ext Z g
  change L.mem (g ≫ f) (R.map (g ≫ f).op a) ↔ L.mem (g ≫ f) (R.map g.op (R.map f.op a))
  simp
