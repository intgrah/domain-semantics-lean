...
theorem rawInterpret_natMotive_value (hM : Γ.as.terms ⊢ M : .nat)
    ...
    (RawFamily.normalizedBodyAction piLimit (Ctx.rawDisplay .nat) (rawInterpret piLimit Γ .nat)
      ...
      (rawInterpret piLimit Γ (C.inst M)).app _ σ.op ρ := by
...
theorem HasFixedness.natRec (hC : .nat :: Γ.as.terms ⊢ C : .sort v) (hM : Γ.as.terms ⊢ M : .nat)
    ...
    HasFixedness Γ (.natRec C M a b) (C.inst M) := by
  intro Γ₁ σ ρ hρ
  rw [rawInterpret_natRec_value piLimit hC hM ha hb,
...
theorem HasEquality.natRec_zero (hC : .nat :: Γ.as.terms ⊢ C : .sort v)
    (ha : Γ.as.terms ⊢ a : C.inst .zero) (hb : Γ.as.terms ⊢ b : Term.natRecType C)
    (hCS : HasSubstitution (Ctx.extension Γ (.nat : Γ.as.terms ⊢ .nat : .type)) C)
    (haF : HasFixedness Γ a (C.inst .zero)) :
...
theorem RawJudgment.natRecDF (hCC' : .nat :: Γ.as.terms ⊢ C ≡ C' : .sort v)
    ...
    (haa' : Γ.as.terms ⊢ a ≡ a' : C.inst .zero)
    (hbb' : Γ.as.terms ⊢ b ≡ b' : Term.natRecType C)
    (hResult : Γ.as.terms ⊢ C.inst M ≡ C'.inst M' : .sort v)
    (pC : RawJudgment (Ctx.extension Γ (.nat : Γ.as.terms ⊢ .nat : .type)) C C' (.sort v))
    (pM : RawJudgment Γ M M' .nat) (pa : RawJudgment Γ a a' (C.inst .zero))
    (pb : RawJudgment Γ b b' (Term.natRecType C))
    (pResult : RawJudgment Γ (C.inst M) (C'.inst M') (.sort v)) :
    RawJudgment Γ (.natRec C M a b) (.natRec C' M' a' b') (C.inst M) where
  ...
  equal := HasEquality.natRec hCC' hMM' haa' hbb' pC.equal pC.right.ren
    pM.left.ideal pM.equal pa.equal pb.equal
  fixed := HasFixedness.natRec hCC'.hasType.1 hMM'.hasType.1
    haa'.hasType.1 hbb'.hasType.1 pC.left.ideal pM.left.ideal pM.fixed pM.left.ren
    pC.left.subst pa.left.ideal pb.left.ideal

theorem RawJudgment.natRec_zero (hC : .nat :: Γ.as.terms ⊢ C : .sort v)
    (ha : Γ.as.terms ⊢ a : C.inst .zero) (hb : Γ.as.terms ⊢ b : Term.natRecType C)
    (pC : RawJudgment (Ctx.extension Γ (.nat : Γ.as.terms ⊢ .nat : .type)) C C (.sort v))
    (pa : RawJudgment Γ a a (C.inst .zero))
    (pRec : RawJudgment Γ (.natRec C .zero a b) (.natRec C .zero a b) (C.inst .zero)) :
    RawJudgment Γ (.natRec C .zero a b) a (C.inst .zero) :=
