...
@[simp] theorem functorHom_map_app (F G : C ⥤ D) {X Y Z : C}
    (f : X ⟶ Y) (x : (F.functorHom G).obj X) (g : Y ⟶ Z) :
    ((F.functorHom G).map f x).app Z g = x.app Z (f ≫ g) := rfl
...
@[ext] theorem HomObj.ext_app {α β : HomObj F G A}
  ...
  HomObj.ext (by funext X a; exact ConcreteCategory.hom_ext _ _ (h X a))

@[simp] theorem HomObj.naturality_apply (α : HomObj F G A)
    {X Y : C} (f : X ⟶ Y) (a : A.obj X) (x : ToType (F.obj X)) :
    α.app Y (A.map f a) (F.map f x) = G.map f (α.app X a x) :=
  by simpa only [ConcreteCategory.comp_apply] using ConcreteCategory.congr_hom (α.naturality f a) x
